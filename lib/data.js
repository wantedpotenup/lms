...
export async function findMemberByNameAndBirth(name, birth) {
  ...
  const targetBirth = normalizeBirthDate(birth);
  if (!targetName || !targetBirth) return null;
  return (
    rows.find(
      (r) =>
        String(r["이름"]).trim() === targetName &&
        normalizeBirthDate(r["생년월일"]) === targetBirth
    ) || null
  );
}
...
export async function getMemberSummary(memberId) {
  ...
  const [{ rows: rawTests }, { rows: rawTestResults }, { rows: rawProjects }] = await Promise.all([
  ...
  const testResults = rawTestResults.map(normalizeTestResultRow);
...
export async function createMember(data) {
  ...
  const birth = formatBirthDateForStorage(data["생년월일"]);
  if (!birth) {
    throw new ValidationError(
      "생년월일 형식을 알아볼 수 없습니다. 1996-02-09, 961229, 19960209 같은 형식으로 입력해주세요."
    );
  }
  const row = {
...
export async function updateMember(memberId, updates) {
  ...
  if (nextUpdates["생년월일"]) {
    const birth = formatBirthDateForStorage(nextUpdates["생년월일"]);
    if (!birth) {
      throw new ValidationError(
        "생년월일 형식을 알아볼 수 없습니다. 1996-02-09, 961229, 19960209 같은 형식으로 입력해주세요."
      );
    }
    nextUpdates["생년월일"] = birth;
  }
  const merged = { ...target, ...nextUpdates, 구성원ID: target["구성원ID"] };
...
export async function bulkCreateMembers(records) {
  ...
  const existingKeys = new Set(
    existingMembers.map(
      (m) => `${String(m["이름"]).trim()}::${normalizeBirthDate(m["생년월일"])}`
    )
  );
  ...
  records.forEach((rec, idx) => {
    ...
    const birthRaw = String(findValueByAliases(rec, ["생년월일", "생일"]) ?? "").trim();
    if (!name || !birthRaw) {
      errors.push(`${line}행: 이름과 생년월일은 필수입니다.`);
      return;
    }
    const birth = formatBirthDateForStorage(birthRaw);
    if (!birth) {
      errors.push(
        `${line}행: 생년월일 형식을 알아볼 수 없습니다 (${name}). 1996-02-09, 961229, 19960209 같은 형식으로 입력해주세요.`
      );
      return;
    }
    const key = `${name}::${normalizeBirthDate(birthRaw)}`;
    ...
    parsedRows.push({
      ...
      상태: findValueByAliases(rec, ["상태"]) || "과정 진행중",
    });
  });
