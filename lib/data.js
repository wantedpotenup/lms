...
import { findValueByAliases, findQuestionColumns, resolveMemberId, findRawValue } from "./uploadMatch";

// "프로젝트평가" 시트를 읽어온 행(rawRow, 실제 헤더 글자가 키)을 정식
// 필드명 기준 객체로 바꾼다. 관리자가 시트 헤더를 보기 좋게 꾸미다가
// (예: "공개여부" -> "공개 여부", "기능구현완성도" -> "기능 구현&안정성(5점)")
// 값을 아예 못 읽어서 화면에 빈 칸/틀린 상태로 보이는 사고를 막기 위함.
function normalizeProjectRow(rawRow) {
  if (!rawRow) return rawRow;
  const out = { __rowNumber: rawRow.__rowNumber };
  for (const field of HEADERS[SHEET_NAMES.PROJECTS]) {
    const aliases = PROJECT_SCORE_ALIASES[field] || [];
...
export async function getMemberSummary(memberId) {
  ...
  const [{ rows: tests }, { rows: testResults }, { rows: rawProjects }] = await Promise.all([
...
export async function updateProject(evalId, updates) {
  ...
  const rawTarget = rows.find((r) => String(r["평가ID"]).trim() === String(evalId).trim());
