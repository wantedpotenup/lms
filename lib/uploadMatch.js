...
import { normalizeBirthDate } from "./format";
...
export function resolveMemberId(rec, members) {
  ...
  const birth = birthRaw ? normalizeBirthDate(birthRaw) : null;
  ...
  const exact = candidates.find((m) => normalizeBirthDate(m["생년월일"]) === birth);
  if (!exact) {
